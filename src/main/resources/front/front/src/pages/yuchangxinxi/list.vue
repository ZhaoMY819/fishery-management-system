		...
		<div class="list-preview">
			<el-form :inline="true" :model="formSearch" class="list-form-pv">
				<el-form-item class="list-item">
					<div class="lable">渔场名称：</div>
					<el-input v-model="formSearch.yuchangmingcheng" placeholder="渔场名称" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">渔场地址：</div>
					<el-input v-model="formSearch.yuchangdizhi" placeholder="渔场地址" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">营业时间：</div>
					<el-input v-model="formSearch.yingyeshijian" placeholder="营业时间" @keydown.enter.native="getList(1, curFenlei)" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<div class="lable">每小时/元：</div>
					<el-input v-model="formSearch.yuchangshoufeistart" placeholder="最小每小时/元" clearable></el-input>
				</el-form-item>
				<el-form-item class="list-item">
					<el-input v-model="formSearch.yuchangshoufeiend" placeholder="最大每小时/元" clearable></el-input>
				</el-form-item>
				<el-button class="list-search-btn" v-if=" true " type="primary" @click="getList(1, curFenlei)">
					<i class="el-icon-search"></i>
					查询
				</el-button>
				<el-button class="list-add-btn" v-if="btnAuth('yuchangxinxi','新增')" type="primary" @click="add('/index/yuchangxinxiAdd')">
						...
						<div class="infoBox">
							...
							<div class="bottomInfo">
								...
								<div class="publisher_item">
									...
									<span class="text">{{item.yuchangzhuzhanghao}}</span>
								</div>
...
<script>
	export default {
		...
		async created() {
			...
			await this.$http.get('option/yuzhongleixing/yuzhongleixing').then(res => {
				if (res.data.code == 0) {
					this.yuzhongleixingOptions = res.data.data;
					this.selectOptionsList.push({name:'鱼种类型',list:this.yuzhongleixingOptions,tableName: 'yuzhongleixing',check: -1})
				}
			});
		...
		methods: {
			...
			getList(page, fenlei, ref = '') {
				...
				let searchWhere = {};
				if (this.formSearch.yuchangmingcheng != '') searchWhere.yuchangmingcheng = '%' + this.formSearch.yuchangmingcheng + '%';
				if (this.formSearch.yuzhongleixing != '') searchWhere.yuzhongleixing = '%' + this.formSearch.yuzhongleixing + '%';
				if (this.formSearch.yuchangdizhi != '') searchWhere.yuchangdizhi = '%' + this.formSearch.yuchangdizhi + '%';
				if (this.formSearch.yingyeshijian != '') searchWhere.yingyeshijian = '%' + this.formSearch.yingyeshijian + '%';
				if(this.formSearch.yuchangshoufeistart!='' && this.formSearch.yuchangshoufeistart!=undefined ){
					searchWhere.yuchangshoufeistart = this.formSearch.yuchangshoufeistart
				}
				if(this.formSearch.yuchangshoufeiend!='' && this.formSearch.yuchangshoufeiend!=undefined){
					searchWhere.yuchangshoufeiend = this.formSearch.yuchangshoufeiend
				}
				if (this.formSearch.quyu != '') searchWhere.quyu = this.formSearch.quyu;
				let user = JSON.parse(localStorage.getItem('sessionForm'))
				if (this.sortType) searchWhere.sort = this.sortType
				if (this.sortOrder) searchWhere.order = this.sortOrder
				this.$http.get(`yuchangxinxi/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
					if (res.data.code == 0) {
			...
			toDetail(item) {
				...
				this.$router.push({path: '/index/yuchangxinxiDetail', query: params});
			},
