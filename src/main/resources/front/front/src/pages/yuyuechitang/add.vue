<template>
	<div class="add-update-preview">
		<el-form
			...
			<el-form-item class="add-item" label="预约图片" v-if="type!='cross' || (type=='cross' && !ro.yuyuetupian)" prop="yuyuetupian">
				<file-upload
					tip="点击上传预约图片"
					action="file/upload"
					:limit="3"
			...
			<el-form-item class="add-item" v-else label="预约图片" prop="yuyuetupian">
				<img v-if="ruleForm.yuyuetupian.substring(0,4)=='http'" class="upload-img" v-bind:key="index" :src="ruleForm.yuyuetupian.split(',')[0]">
				<img v-else class="upload-img" v-bind:key="index" v-for="(item,index) in ruleForm.yuyuetupian.split(',')" :src="baseUrl+item">
			</el-form-item>
			...
			<el-form-item class="add-item" label="总时长" prop="zongshizhang">
				<el-input v-model="zongshizhang" placeholder="总时长" disabled></el-input>
			</el-form-item>
			<el-form-item class="add-item" label="总费用" prop="zongfeiyong">
			...
			<el-form-item class="add-item" label="会员折扣" prop="huiyuanzhekou">
				<el-input-number v-model="ruleForm.huiyuanzhekou" placeholder="会员折扣" :disabled=" false ||ro.huiyuanzhekou"></el-input-number>
			</el-form-item>
	...
	export default {
		...
		computed: {
			...
			zongfeiyong : {
				get: function () {
					let c = this.ruleForm
					let a = c.zongshizhang*c.yuchangshoufei*c.huiyuanzhekou
		...
		methods: {
			...
			init(type) {
				...
				if(type=='cross'){
					...
					for (var o in obj){
						...
						if(o=='yuyuetupian'){
							this.ruleForm.yuyuetupian = obj[o]?obj[o].split(",")[0]:'';
			...
			// 多级联动参数
			info() {
				this.$http.get(`yuyuechitang/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
					if (res.data.code == 0) {
			...
			async onSubmit() {
				...
				if(this.ruleForm.zongfeiyong==0){
					this.$message.error('总费用不能为空')
					return false
				}
				...
				await this.$refs["ruleForm"].validate(async valid => {
					if(valid) {
						...
						await this.$http.post(`yuyuechitang/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(async res => {
							if (res.data.code == 0) {
								this.$message({
									message: '操作成功',
									type: 'success',
			...
			yuyuetupianUploadChange(fileUrls) {
				this.ruleForm.yuyuetupian = fileUrls.replace(new RegExp(this.$config.baseUrl,"g"),"");
			},
		}
	};
</script>
