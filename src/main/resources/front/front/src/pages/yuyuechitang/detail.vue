	...
	<!--  -->
		<div class="breadcrumb-preview">
			<el-breadcrumb :separator="''">
				...
				<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" :to="'/index/yuyuechitang?centerType=' + (centerType?'1':'0')"><a>{{item.name}}</a></el-breadcrumb-item>
		...
		<div class="detail-preview">
			<div class="attr">
				<div class="info">
					...
					<div class="item">
						<div class="lable">总时长</div>
						<div class="text "  >{{detail.zongshizhang}}</div>
					</div>
					<div class="item">
						<div class="lable">总费用</div>
						<div class="text "  >{{detail.zongfeiyong}}</div>
					</div>
					<div class="item">
					...
					<div class="item">
						<div class="lable">会员折扣</div>
						<div class="text "  >{{detail.huiyuanzhekou}}</div>
					</div>
					<div class="item">
					...
					<div class="btn_box">
						<el-button class="editBtn" v-if="btnAuth('yuyuechitang','修改')&&detail.sfsh!='是'&&detail.sfsh!='否' " @click="editClick">修改</el-button>
						<el-button class="delBtn" v-if="btnAuth('yuyuechitang','删除')" @click="delClick">删除</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yuyuechitang','预约取消')" 
							@click="onAcross('yuyuequxiao','否','否','','')" type="warning">
							预约取消
						</el-button>
						<el-button class="reserveBtn" 
							v-if="btnAuth('yuyuechitang','回鱼记录')" 
							@click="onAcross('huiyujilu','是','是','','')" type="warning">
							回鱼记录
						</el-button>
						<el-button class="auditBtn" v-if="btnAuth('yuyuechitang','审核') && (detail.sfsh=='待审核'||!detail.sfsh)" @click="onSH" type="warning">审核</el-button>
						<el-button class="payBtn" v-if="(detail.ispay=='未支付'||!detail.ispay)&&btnAuth('yuyuechitang','支付')" type="warning" size="small" @click="payClick">支付</el-button>
					</div>
				</div>
			</div>
	...
	export default {
		...
		methods: {
			...
			init() {
				...
				this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
					if (res.data.code == 0) {
						...
						if(this.detail.yuyuetupian) {
							this.detailBanner = this.detail.yuyuetupian.split(",w").length>1?[this.detail.yuyuetupian]:this.detail.yuyuetupian.split(',');
						}
						this.$forceUpdate();
			...
			async onAcross(acrossTable,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue,type=1){
				...
				localStorage.setItem('crossTable',`yuyuechitang`);
			...
			payClick(){
				...
				localStorage.setItem('paytable','yuyuechitang')
			...
			backClick(){
				if(this.storeupType){
					...
					this.$router.push({path: '/index/yuyuechitang', query: params});
				}
			},
			// 下载
			download(file ){
			...
			// 修改
			editClick(){
				this.$router.push(`/index/yuyuechitangAdd?type=edit&&id=${this.detail.id}`);
			},
			// 删除
			async delClick(){
				await this.$confirm('是否删除此预约池塘？') .then(_ => {
					this.$http.post('yuyuechitang/delete', [this.detail.id]).then(async res => {
						if (res.data.code == 0) {
							this.$message({
								type: 'success',
								message: '删除成功!',
