	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="upload" v-if="type!='info' && !ro.huiyutupian" label="回鱼图片" prop="huiyutupian" >
					<file-upload
						tip="点击上传回鱼图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item class="upload" v-else-if="ruleForm.huiyutupian" label="回鱼图片" prop="huiyutupian" >
					<img v-if="ruleForm.huiyutupian.substring(0,4)=='http'&&ruleForm.huiyutupian.split(',w').length>1" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.huiyutupian" width="100" height="100">
					<img v-else-if="ruleForm.huiyutupian.substring(0,4)=='http'" class="upload-img" style="margin-right:20px;" v-bind:key="index" :src="ruleForm.huiyutupian.split(',')[0]" width="100" height="100">
					<img v-else class="upload-img" style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.huiyutupian.split(',')" :src="$base.url+item" width="100" height="100">
				</el-form-item>
				<el-form-item class="select" v-if="type!='info'" label="鱼种类型" prop="yuzhongleixing" >
					<el-select :disabled="ro.yuzhongleixing" @change="yuzhongleixingChange" v-model="ruleForm.yuzhongleixing" placeholder="请选择鱼种类型">
						<el-option
				...
				<el-form-item class="input" v-else-if="ruleForm.yuzhongleixing" label="鱼种类型" prop="yuzhongleixing" >
					<el-input v-model="ruleForm.yuzhongleixing" placeholder="鱼种类型" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="回鱼价格" prop="huiyujiage" >
					<el-input-number v-model="ruleForm.huiyujiage" placeholder="回鱼价格" :disabled="ro.huiyujiage"></el-input-number>
				</el-form-item>
				<el-form-item v-else class="input" label="回鱼价格" prop="huiyujiage" >
					<el-input v-model="ruleForm.huiyujiage" placeholder="回鱼价格" readonly></el-input>
				</el-form-item>
				<el-form-item class="input" v-if="type!='info'"  label="重量" prop="zhongliang" >
					<el-input v-model.number="ruleForm.zhongliang" placeholder="重量" clearable  :readonly="ro.zhongliang"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="重量" prop="zhongliang" >
				...
				<el-form-item class="date" v-if="type!='info'" label="回鱼时间" prop="huiyushijian" >
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.huiyushijian" 
						type="date"
						:readonly="ro.huiyushijian"
						placeholder="回鱼时间"
					></el-date-picker> 
				</el-form-item>
				<el-form-item class="input" v-else-if="ruleForm.huiyushijian" label="回鱼时间" prop="huiyushijian" >
					<el-input v-model="ruleForm.huiyushijian" placeholder="回鱼时间" readonly></el-input>
				</el-form-item>
			...
			<el-form-item v-else-if="ruleForm.xiangqing" label="详情" prop="xiangqing" >
				<span class="text">{{ruleForm.xiangqing}}</span>
			</el-form-item>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='huiyutupian'){
							this.ruleForm.huiyutupian = obj[o];
			...
			yuzhongleixingChange () {
				this.$http({
					url: `follow/yuzhongleixing/yuzhongleixing?columnValue=`+ this.ruleForm.yuzhongleixing,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
						if(data.data.huiyujiage){
							this.ruleForm.huiyujiage = data.data.huiyujiage
						}
					} else {
						this.$message.error(data.msg);
					}
			...
			async onSubmit() {
					...
					if(this.ruleForm.huiyutupian!=null) {
						this.ruleForm.huiyutupian = this.ruleForm.huiyutupian.replace(new RegExp(this.$base.url,"g"),"");
					}
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `huiyujilu/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.huiyujiluCrossAddOrUpdateFlag = false;
											this.parent.search();
			...
			huiyutupianUploadChange(fileUrls) {
				this.ruleForm.huiyutupian = fileUrls;
			},
		}
	};
</script>
