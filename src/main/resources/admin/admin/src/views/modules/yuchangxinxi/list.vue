<template>
	<div class="main-content" :style='{"padding":"20px 30px","fontSize":"15px"}'>
			...
			<el-form class="center-form-pv" :style='{"margin":"0 10px 20px"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","background":"#fff","display":"flex"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">渔场名称</label>
						<el-input v-model="searchForm.yuchangmingcheng" placeholder="渔场名称" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="鱼种类型" prop="yuzhongleixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">鱼种类型</label>
						<el-select clearable v-model="searchForm.yuzhongleixing" placeholder="请选择鱼种类型" multiple>
							<el-option v-for="(item,index) in yuzhongleixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">渔场地址</label>
						<el-input v-model="searchForm.yuchangdizhi" placeholder="渔场地址" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">营业时间</label>
						<el-input v-model="searchForm.yingyeshijian" placeholder="营业时间" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">每小时/元</label>
						<el-input v-model="searchForm.yuchangshoufeistart" placeholder="最小每小时/元" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' :label="'至'">
						<el-input v-model="searchForm.yuchangshoufeiend" placeholder="最大每小时/元" clearable></el-input>
					</div>
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="区域" prop="quyu">
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0","flexWrap":"wrap","background":"#fff","display":"flex"}'>
					<el-button class="add" v-if="isAuth('yuchangxinxi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia14" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('yuchangxinxi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
						<span class="icon iconfont icon-shanchu6" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","height":"34px"}'></span>
						删除
					</el-button>


					<el-button class="btn18" v-if="isAuth('yuchangxinxi','渔场类型统计')" type="success" @click="chartDialog1()">
			...
			<div :style='{"width":"100%","padding":"0 10px","fontSize":"15px","color":"#000"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="yuchangmingcheng"
						label="渔场名称">
						<template slot-scope="scope">
							{{scope.row.yuchangmingcheng}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="yuzhongleixing"
					...
					<el-table-column :resizable='true' :sortable='true' prop="yuchangdizhi" label="渔场地址">
						<template slot-scope="scope">
							{{ scope.row.yuchangdizhi?scope.row.yuchangdizhi.split(';')[2]:'' }}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="yingyeshijian"
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="yuchangzhuzhanghao"
						label="渔场主账号">
						<template slot-scope="scope">
							{{scope.row.yuchangzhuzhanghao}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="lianxifangshi"
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('yuchangxinxi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看
							</el-button>
							<el-button class="btn8" v-if="isAuth('yuchangxinxi','预约')" @click="yuyuechitangCrossAddOrUpdateHandler(scope.row,'cross','','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								预约
							</el-button>
							<el-button class="edit" v-if=" isAuth('yuchangxinxi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								修改
							</el-button>

							<el-button class="btn8" v-if="isAuth('yuchangxinxi','私聊')" type="success" @click="chatClick(scope.row)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								私聊
							</el-button>
							<el-button class="view" v-if="isAuth('yuchangxinxi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('yuchangxinxi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"40px"}'></span>
								删除
							</el-button>
						</template>
		...
		<yuyuechitang-cross-add-or-update v-if="yuyuechitangCrossAddOrUpdateFlag" :parent="this" ref="yuyuechitangCrossaddOrUpdate"></yuyuechitang-cross-add-or-update>
		...
		<el-dialog
			...
			width="800">
			<div id="yuzhongleixingChart1" style="width:100%;height:600px;"></div>
...
<script>
	...
	import yuyuechitangCrossAddOrUpdate from "../yuyuechitang/add-or-update";
	...
	export default {
		...
		data() {
			return {
				...
				yuyuechitangCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			yuyuechitangCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.yuyuechitangCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','yuchangxinxi');
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.yuyuechitangCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.yuyuechitangCrossaddOrUpdate.init(row.id,type);
				});
			},
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var yuzhongleixingChart1 = echarts.init(document.getElementById("yuzhongleixingChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							yuzhongleixingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								yuzhongleixingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.yuchangmingcheng!='' && this.searchForm.yuchangmingcheng!=undefined){
					params['yuchangmingcheng'] = '%' + this.searchForm.yuchangmingcheng + '%'
				}
				if(this.searchForm.yuzhongleixing&&this.searchForm.yuzhongleixing.length){
					let yuzhongleixingArr = []
					for(let i in this.searchForm.yuzhongleixing){
						yuzhongleixingArr.push('%' + this.searchForm.yuzhongleixing[i] + '%')
					}
					params['yuzhongleixing'] = yuzhongleixingArr.join(',')
				}
 				if(this.searchForm.yuchangdizhi!='' && this.searchForm.yuchangdizhi!=undefined){
					params['yuchangdizhi'] = '%' + this.searchForm.yuchangdizhi + '%'
				}
 				...
 				if(this.searchForm.yuchangshoufeistart!='' && this.searchForm.yuchangshoufeistart!=undefined ){
					params['yuchangshoufeistart'] = this.searchForm.yuchangshoufeistart
				}
				if(this.searchForm.yuchangshoufeiend!='' && this.searchForm.yuchangshoufeiend!=undefined){
					params['yuchangshoufeiend'] = this.searchForm.yuchangshoufeiend
				}
			...
			disscussListHandler(id,type) {
				this.$router.push({path:'/discussyuchangxinxi',query:{refid:id}});
			},
			// 删除
