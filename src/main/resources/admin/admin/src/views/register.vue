	...
	<div>
		<div class="register-container">
			<el-form v-if="pageFlag=='register'" ref="ruleForm" class="rgs-form animate__animated animate__backInDown" :model="ruleForm" :rules="rules">
				<div class="rgs-form2">
					<div class="title">基于Spring Boot的钓手渔场管理系统设计与实现注册</div>
					...
					<el-form-item class="list-item" v-if="tableName=='yuchangzhu'">
						<div class="lable" :class="changeRules('yuchangzhuzhanghao')?'required':''">渔场主账号：</div>
						<el-input  v-model="ruleForm.yuchangzhuzhanghao"  autocomplete="off" placeholder="渔场主账号"  type="text"  />
					</el-form-item>
					<el-form-item class="list-item" v-if="tableName=='yuchangzhu'">
						<div class="lable" :class="changeRules('mima')?'required':''">密码：</div>
						<el-input  v-model="ruleForm.mima"  autocomplete="off" placeholder="密码"  type="password"  />
...
export default {
	...
	methods: {
		...
		login() {
			...
			if((!this.ruleForm.yuchangzhuzhanghao) && `yuchangzhu` == this.tableName){
				this.$message.error(`渔场主账号不能为空`);
				return
			}
			if((!this.ruleForm.mima) && `yuchangzhu` == this.tableName){
				this.$message.error(`密码不能为空`);
				return
			}
			if((this.ruleForm.mima!=this.ruleForm.mima2) && `yuchangzhu` == this.tableName){
				this.$message.error(`两次密码输入不一致`);
				return
			}
            if(this.ruleForm.touxiang!=null) {
			...
			if(`yuchangzhu` == this.tableName && this.ruleForm.lianxifangshi &&(!this.$validate.isMobile(this.ruleForm.lianxifangshi))){
